...
EDGE_CASE_PATH = "data/edge_case_queries.jsonl"
...
class EdgeCaseResponseGenerator:
    def __init__(self):
        self.llm = None
    ...
    def generate_all_responses(self):
        ...
        print(f"Generating responses for {len(edge_cases)} edge case queries")
        
        for i, edge_case in enumerate(edge_cases):
            print(f"Processing query {i+1}/{len(edge_cases)}: {edge_case['user_query'][:50]}")
            
            try:
                response = self.generate_response(edge_case['user_query'])
                edge_case['agent_reply'] = response
                print(f"  Response: {response[:100]}")
            except Exception as e:
                print(f"  Error: {e}")
                edge_case['agent_reply'] = f"ERROR: {e}"
        
        with open(EDGE_CASE_PATH, 'w') as f:
            for edge_case in edge_cases:
                f.write(json.dumps(edge_case) + '\n')
        
        print(f"All responses saved to {EDGE_CASE_PATH}")

def main():
    generator = EdgeCaseResponseGenerator()
    
    generator.load_model()
    generator.products = generator.load_products()
    generator.build_embeddings()
    generator.load_tool_data()
    
    generator.generate_all_responses()
